<?php
# Usage: php -S localhost:8000 -t public/web misc/router.php
# https://github.com/darklightcode/PHP-Built-in-web-server-Router

$headers = ['Accept', 'Accept-CH', 'Accept-Charset', 'Accept-Datetime', 'Accept-Encoding', 'Accept-Ext', 'Accept-Features', 'Accept-Language', 'Accept-Params', 'Accept-Ranges',
...
class PHP_Webserver_Router
{
    ...
    var $accepted_extensions_as_root = ["php", "html", "htm"];
    ...
    {
        $formatVarsToUnix = ['DOCUMENT_ROOT', 'SCRIPT_FILENAME', 'SCRIPT_NAME', 'PHP_SELF'];
...
//                echo($this->physical_file);
//                echo __METHOD__;
//                $s = file_get_contents($this->physical_file);
//                echo $s;
                exit;
            }
        }
    ...
    function autoDetectIndex()
    {
        ...
        if (!file_exists($indexRoot)) {
            $viableFilesForIndex = ['index.php', 'index.phtml', 'index.html', 'index.htm', 'index.html5', 'index.php5'];
    ...
    private function create_mime_file()
    {
        ...
        foreach ($s as $k => $v) {
            ...
            $new_val = trim(str_replace(["   '", "'"], "", $uri_path[1]));
